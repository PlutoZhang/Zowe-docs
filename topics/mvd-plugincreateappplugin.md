...
A Project Giza plug-in is a separately installable collection of files that can provide a set of web resources \(JavaScript, HTML, and so on\) that can be presented in the user interface as window \(or as an embedded panel\) or a set of RESTful services, or both RESTful services and user interfaces.

Before you can begin building a Project Giza plug-in, you must set the environment variables that are necessary to support the plug-in environment.
